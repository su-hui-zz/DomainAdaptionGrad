...
class ConvBlock(nn.Module):
    maml = False #Default
    def __init__(self, indim, outdim, pool = True, stride = 1, padding = 1):
        super(ConvBlock, self).__init__()
        self.indim  = indim
        self.outdim = outdim
        if self.maml:
            self.C      = Conv2d_fw(indim, outdim, 3, stride = stride, padding = padding)
            self.BN     = BatchNorm2d_fw(outdim)
        else:
            self.C      = nn.Conv2d(indim, outdim, 3, stride = stride, padding= padding)
...
class ConvNet224(nn.Module):
    def __init__(self, depth, flatten = True):
        ...
        for i in range(depth):
            ...
            outdim = 64
            if i < 2:
                B = ConvBlock(indim, outdim, stride = 2, pool = ( i <4 ) )
            else:
    ...
    def forward(self,x,get_features=False):
        if get_features == True:
            outputs = []
            for name, module in self.trunk._modules.items(): # 注意，最后一个是flatten，所以倒数第一个和倒数第二个内容一致
                x = module(x)
                outputs.append(x)
            return outputs
        else:
...
class ConvNetNopool(nn.Module): #Relation net use a 4 layer conv with pooling in only first two layers, else no pooling
    ...
    def forward(self,x,get_features=False):
        if get_features == True:
            outputs = []
            for name, module in self.trunk._modules.items():
                x = module(x)
                outputs.append(x)
            return outputs
        else:
